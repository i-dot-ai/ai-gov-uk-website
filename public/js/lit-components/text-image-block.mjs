...
(async () => {
  ...
  const TextImageBlock = class extends LitElement {
    ...
    render() {
      ...
      image = html`
        ${this.imageCaption ? html`<figure>` : ''}
        <img class="block mt-4 w-full lg:!mt-0" src=${this.image} alt=${this.imageAlt}/>
        ${this.imageCaption ? html`
            <figcaption class="flex mt-2 text-sm text-slate-600">
              <!-- https://www.svgrepo.com/vectors/image/ -->
      ...
      if (this.side === "right") {
        mainHtml = html`
          ...
          <!-- mobile image - because it appears below the text -->
          <div class="lg:hidden">
            ${image}
          </div>
        `;
      ...
      return html`
        <div class="text-image-block gap-8 grid-cols-2 items-center mt-4 lg:grid">
          ${mainHtml}
        </div>
      `;
    }
  ...
  customElements.define("text-image-block", TextImageBlock);
